...
const getBuyerCredentials = () => {
  const buyerEmail = process.env.MARKETPLACE_BUYER_EMAIL || process.env.MARKETPLACE_EMAIL || 'acamet117@gmail.com';
  const buyerPassword = process.env.MARKETPLACE_BUYER_PASSWORD || process.env.MARKETPLACE_PASSWORD || '123456';

  return { buyerEmail, buyerPassword };
};

const getSellerCredentials = () => ({
  sellerEmail: process.env.MARKETPLACE_SELLER_EMAIL,
  sellerPassword: process.env.MARKETPLACE_SELLER_PASSWORD
});
